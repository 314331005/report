...
public class DBToolkit2 { 
        private static final Log log = LogFactory.getLog(DBToolkit2.class); 
        ...
        static { 
                try { 
                        props.load(DBToolkit2.class.getResourceAsStream("/jdbc.properties")); 
                } catch (IOException e) { 
                		e.printStackTrace();
                        log.error("#ERROR# :系统加载sysconfig.properties配置文件异常，请检查！", e); 
                } 
                url = (props.getProperty("jdbc2.url")); 
                username = (props.getProperty("jdbc2.username")); 
                password = (props.getProperty("jdbc2.password")); 
                driver = (props.getProperty("jdbc2.driver")); 
                //注册驱动类 
                try { 
